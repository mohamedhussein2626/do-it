...
export async function createPodcast(fileId: string) {
  try {
    ...
    // For podcasts, always extract from PDF with maxPages: 4 to limit ElevenLabs API usage
    // This ensures we only use the first 4 pages regardless of existing chunks
    if (file.key && file.fileType === 'application/pdf') {
      console.log("📄 PDF file detected - extracting first 4 pages for podcast...");
      console.log(`📁 File key: ${file.key}`);
      
      try {
        // Import required modules
        const { GetObjectCommand } = await import("@aws-sdk/client-s3");
        const { r2Client, R2_BUCKET_NAME } = await import('./r2-config');
        const { processHybridPdf } = await import('./pdf-ocr-hybrid');
        ...
        // Extract text using hybrid PDF processor - limit to first 4 pages for podcasts
        const extractedText = await processHybridPdf(buffer, {
          extractImageText: false, // Faster extraction without OCR
          maxPages: 4, // Only use first 4 pages for podcasts to reduce ElevenLabs API usage
        });

        if (extractedText && extractedText.trim()) {
          fileContent = extractedText;
          console.log(`✅ Extracted first 4 pages: ${fileContent.length} characters`);
        } else {
          throw new Error("No text extracted from PDF");
        }
      } catch (pdfError) {
        console.error("❌ Error extracting from PDF:", pdfError);
        // Fallback to chunks if PDF extraction fails
        console.log("⚠️ Falling back to chunks...");
        
        const chunks = await db.chunk.findMany({
          where: { fileId: file.id },
          take: 8, // Limit to ~8 chunks (approx 4 pages, ~2 chunks per page)
          orderBy: { createdAt: "asc" },
        });
        
        if (chunks.length > 0) {
          fileContent = chunks.map((chunk: Chunk) => chunk.text).join("\n\n");
          console.log(`✅ Using limited chunks (${chunks.length}) for content`);
        } else {
          return {
      ...
      // For non-PDF files or files without key, use chunks
      const chunks = await db.chunk.findMany({
        where: { fileId: file.id },
