...
Generate the flashcards now:`;
    ...
    while (!cards && attempts < maxAttempts) {
      ...
      try {
        const response = await openai.chat.completions.create({
          ...
          temperature: 0.1,
          max_tokens: 1200, // Reduced to fit within available credits
          messages: [
            {
              role: "system",
              content:
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes("402") || (error as any)?.status === 402 || (error as any)?.code === 402) {
        ...
        errorMessage = "Failed to generate flashcards using AI. Please check your API key and try again.";
      } else if (error.message.includes("database") || error.message.includes("Prisma")) {
        errorMessage = "Failed to save flashcards to database. Please try again.";
      } else if (error.message.includes("timeout") || error.message.includes("Timeout")) {
        errorMessage = "Request timed out. Please try again.";
      } else {
        errorMessage = `Failed to create flashcards: ${error.message.substring(0, 100)}`;
      }
    } else {
